...
def probability_psi_approx(psi_o, psi, c, r, approximate=1000):
    m = r/c
...
def L_statistic_vec(psi_o_array, psi_a_array, psi_null, mrna_array, c, min_probability, sum_times, approximate):
    L = []
    psi_a_array = np.array([0.99 if x > 0.99 else 0.01 if x < 0.01 else x for x in psi_a_array])

    func = lambda i: L_observation(psi_o_array[i], psi_a_array[i], psi_null, mrna_array[i], c, min_probability, sum_times, approximate)
...
def calculate_exon_L(PSI_tab, W, mrna_counts, exon, k = 0, c = 0.1, weight_distance=True, randomize = False, seed=0, min_probability = 0.00001, sum_times=10, approximate = 0):
    ...
    try:
        ...
        L_vec = L_statistic_vec(psi_o_array, psi_a_array, psi_null, mrna_array, c, min_probability, sum_times, approximate)
